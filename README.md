...
- 문제 : 유저마다 다른 네트워크 환경으로 인해 일관된 동기화 타이밍을 잡기 어려움
- 해결 : 주기적인 타임 동기화 패킷을 통해 `RTT`를 실시간으로 측정하고, 이를 기반으로 서버와 클라이언트 간의 시간 오차를 보정하는 동기화 로직 구현

### Entity Interpolation (개체 보간)

- 문제 : 서버로부터 수신되는 패킷의 간격이 일정하지 않아 타 플레이어의 움직임이 툭툭 끊겨 보이는 현상 발생
- 해결 : 수신된 패킷 데이터를 즉시 반영하지 않고, `RTT`를 활용하여 과거의 위치 데이터들 사이를 부드럽게 보간하여 출력하는 알고리즘을 적용해 가시적인 떨림 현상 해결

### Server Reconciliation (서버 측 재조정)

- 문제 : 클라이언트 측 예측을 적용할 경우, 서버의 판전 결과와 클라이언트의 위치가 어긋날 때 캐릭터가 순간이동하는 현상 발생
- 해결 : 서버로부터 수신된 마지막 확정 위치를 기준으로 클라이언트의 미처 처리되지 않은 입력값들을 다시 시뮬레이션하여, 이질감 없는 위치 교정 프로세스 구축.
